...
public class PinnedNotesOverlay {
    ...
    private static void drawSticker(GuiGraphics gfx, Minecraft mc, Sticker s, int mx, int my) {
        ...
        int headerColor2 = (alpha << 24) | (darken(s.color, 0.82f) & 0x00FFFFFF);
        gfx.fill(x + 3, y + 3, x + w + 3, y + h + 3, s.transparent ? 0x22000000 : 0x44000000);
        gfx.fill(x, y, x + w, y + h, bodyColor);
        gfx.fill(x, y, x + w, y + HEADER, headerColor2);
        ...
        // Кнопка прозрачности (полукруг / eye icon — используем "o" как глаз)
        int tx2 = cx - HEADER;
        ...
        // Текст стикера с MD-рендерингом
        if (note != null && !note.content.isEmpty()) {
            int ty = y + HEADER + PAD;
            alpha = s.transparent ? 0x88 : 0xFF;
    ...
    private static void drawStickerLine(GuiGraphics gfx, Minecraft mc, Sticker s,
        ...
        if (line.startsWith("# ")) {
            String t = mc.font.plainSubstrByWidth(line.substring(2), w - PAD * 2);
            ...
            gfx.drawString(mc.font, mc.font.plainSubstrByWidth(line.substring(3), w - PAD * 2), x + PAD, ty, (alpha << 24) | 0x005A3800, false);
        } else if (line.startsWith("### ")) {
            gfx.drawString(mc.font, mc.font.plainSubstrByWidth(line.substring(4), w - PAD * 2), x + PAD, ty, (alpha << 24) | 0x009B8A6A, false);
        } else if (line.equals("---")) {
            int mid = ty + LINE_H / 2;
            gfx.fill(x + PAD, mid, x + w - PAD, mid + 1, (alpha << 24) | 0x008B7355);
        } else if (line.startsWith("> ")) {
            gfx.fill(x + PAD, ty, x + PAD + 2, ty + LINE_H - 1, (alpha << 24) | 0x008B7355);
            gfx.drawString(mc.font, mc.font.plainSubstrByWidth(line.substring(2), w - PAD * 2 - 4), x + PAD + 4, ty, (alpha << 24) | 0x007A6A50, false);
        } else if (line.startsWith("`") && line.endsWith("`") && line.length() > 1) {
            String code = line.substring(1, line.length() - 1);
            String fit = mc.font.plainSubstrByWidth(code, w - PAD * 2 - 4);
            ...
            gfx.drawString(mc.font, mc.font.plainSubstrByWidth(line, w - PAD * 2), x + PAD, ty, ink, false);
        }
    }
    ...
    public static boolean handlePress(double rawX, double rawY, Minecraft mc) {
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            ...
            // Клик по чекбоксу todo в стикере
    ...
    private static boolean tryToggleTodoInSticker(Sticker s, int mx, int my) {
        ...
        for (String line : note.content.split("\n")) {
            ...
            if (line.startsWith("[ ] ") || line.startsWith("[x] ")) {
                // Зона чекбокса: маленький квадрат
                if (mx >= x + PAD && mx < x + PAD + 9 && my >= ty && my < ty + 8) {
                    // Находим позицию в content и переключаем
    ...
    private static int darken(int color, float f) {
        ...
        int g2 = (int) (Math.min(255, ((color >> 8) & 0xFF) * f));
